...
module  Math
{
    ...
    ensures !(exists x' {:trigger power(x' as nat,2)} :: 
                    x'>x && power(x' as nat,2) <= n as nat)
    {
        x:=n;    
        var y:uint64 :=(x+1)/2;
        ...
        while(y<x)
            decreases x
            invariant y < x <==> power(x as nat,2) > n as nat;
        ...
        {   
            ...
            LemmaLoopInvariants(x as nat,n as nat);
        }

        LemmaSquareIsMonotonic(x as nat +1);
