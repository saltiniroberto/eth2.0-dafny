 ...
 module SSZ_Merkleise {
        ...
        ensures 0 <= chunkCount(s) // add upper limit ???
    {
        match s
            ...
            case Uint64(_) => chunkCountUint64(s)
    ...
    function method chunkCountUint64(n: Serialisable): nat
        // all basic types require 1 leaf (reference: simple-serialize.md)
        requires n.Uint64?
        ensures chunkCountUint64(n) ==  1
        ensures |pack(n)| == chunkCountUint64(n)
    {
        1
    }
    ...
    function method pack(s: Serialisable): seq<chunk>
        requires typeOf(s) in {Bool_,Bytes32_, Uint64_}
    {
        match s
            case Bool(b) => packBool(b)
            case Uint64(_) => packUint64(s)
    ...
    function method packUint64(s: Serialisable): seq<chunk>
        requires s.Uint64?
        ensures |packUint64(s)| == 1
    {
